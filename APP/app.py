...
def insert_csv_to_db(df, table_name):
    ...
    # Potong string jika lebih dari 200 karakter
    for col in df.columns:
        df[col] = df[col].apply(truncate_string)
    ...
    if table_name == 'laporan':
        ...
        """
        data = [tuple(x) for x in df[['no_laporan', 'uid_dinas', 'no_tiket_dinas', 'dinas', 'l2_notes', 
                                      'status', 'tiket_dibuat', 'tiket_selesai', 'durasi_penanganan']].values]
    elif table_name == 'log_dinas':
        insert_query = """
        INSERT INTO log_dinas (
        ...
        WHERE no_tiket_dinas = %s AND status = %s AND catatan = %s
        """
        data = [tuple(x) for x in df[['no_laporan', 'no_tiket_dinas', 'dinas', 'agent_l2', 'status', 
                                      'waktu_proses', 'durasi_penanganan', 'catatan', 'foto_1', 
                                      'foto_2', 'foto_3', 'foto_4']].values]
    ...
    for row in data:
        if table_name == 'log_dinas':
            duplicate = conn.query(check_query, (row[1], row[4], row[7]))
            if duplicate.iloc[0, 0] == 0:
                valid_data.append(row)
            else:
                duplicate_count += 1
        else:
    ...
    try:
        if valid_data:
            conn.query(insert_query, valid_data)
